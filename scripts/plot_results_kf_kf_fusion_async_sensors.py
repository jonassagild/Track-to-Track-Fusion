...
plots results after performing Kalman filter fusion on unsynchronized sensors
"""
import numpy as np
import scipy
...
generate_scenario_3(seed=seed, permanent_save=False, radar_meas_rate=radar_meas_rate, ais_meas_rate=ais_meas_rate,
                    sigma_process=sigma_process, sigma_meas_radar=sigma_meas_radar, sigma_meas_ais=sigma_meas_ais,
                    timesteps=timesteps)
...
kf_kf_fusion = KFFusionUnsyncSensors(start_time, prior,
...
measurement_model_radar = kf_kf_fusion.measurement_model_radar
measurement_model_ais = measurement_model_radar

tracks_fused, tracks_radar = kf_kf_fusion.track(measurements_radar, measurements_ais, estimation_rate=1)
...
# calculate NEES
nees_fused = calc_metrics.calc_nees(tracks_fused, ground_truth)
nees_radar = calc_metrics.calc_nees(tracks_radar, ground_truth)
# calculate ANEES
anees_fused = calc_metrics.calc_anees(nees_fused)
anees_radar = calc_metrics.calc_anees(nees_radar)
# calculate RMSE
rmse_fused = calc_metrics.calc_rmse(tracks_fused, ground_truth)
rmse_radar = calc_metrics.calc_rmse(tracks_radar, ground_truth)
# calculate percentage NEES within CI
percentage_nees_fused_within_CI = calc_metrics.calc_percentage_nees_within_ci(nees_fused, ci_nees)
percentage_nees_radar_within_CI = calc_metrics.calc_percentage_nees_within_ci(nees_radar, ci_nees)

print("Percentage NEES radar within CI: " + str(percentage_nees_radar_within_CI))
metrics_info = "Percentage NEES within CI: " + str(percentage_nees_fused_within_CI) + "\n" + "RMSE: " + str(rmse_fused) + \
               "\n" + "ANEES: " + str(anees_fused) + "\n" + "Alpha: " + str(alpha) + "\n" + "CI NEES: " + str(ci_nees) + \
...
ax.legend()
ax.set_title("Kalman filter tracking and kalman filter fusion ")
fig.show()
if save_fig:
